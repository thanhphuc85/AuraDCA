...
- **Agent-stack upgrade — Circle Nanopayments (x402) on Arc — first slice shipped.** Beyond *spending* USDC to accumulate, the agent *pays for its own inputs*: a metered "premium market brief" behind an **HTTP-402** paywall ([`api/x402-brief.ts`](api/x402-brief.ts)), unlocked by a signed **USDC payment authorization** (EIP-3009 `transferWithAuthorization`, the exact `exact`-scheme struct a facilitator settles). This is a genuinely different money primitive from the DCA swap — *pay-per-call for a service*, not accumulate a token — and it's built as an **additive module** ([`src/x402/`](src/x402/)) that never touches the DCA pipeline: 12 unit tests cover sign/verify/reject, and `npm run x402-demo` runs the full 402→pay→200 handshake reproducibly (offline, or against a deployed endpoint). **Honest scope:** the authorization is cryptographically *verified* but **settlement is gated off** — no on-chain broadcast yet — the same paper-fill honesty we applied to the cirBTC outage. Productionizing to on-chain settle (via Circle Gateway) and to signing with the agent's Circle Developer-Controlled Wallet is the next step; the signed struct is already settlement-ready.
