...
- **Nâng cấp agent stack — Circle Nanopayments (x402) trên Arc — slice đầu đã ship.** Ngoài việc *chi* USDC để tích luỹ, agent *tự trả cho đầu vào của chính nó*: một "premium market brief" tính phí sau paywall **HTTP-402** ([`api/x402-brief.ts`](api/x402-brief.ts)), mở khoá bằng một **uỷ quyền thanh toán USDC** đã ký (EIP-3009 `transferWithAuthorization`, đúng struct scheme `exact` mà facilitator settle). Đây là **primitive khác** với swap DCA — trả-theo-lượt-gọi cho một dịch vụ, không phải tích luỹ token — và được xây như **module cộng thêm** ([`src/x402/`](src/x402/)), không đụng tới pipeline DCA: 12 unit test phủ ký/verify/từ chối, và `npm run x402-demo` chạy trọn handshake 402→pay→200 tái lập được (offline, hoặc trỏ vào endpoint đã deploy). **Nói rõ phạm vi:** uỷ quyền được **verify** bằng mật mã nhưng **settlement đang tắt** — chưa broadcast on-chain — đúng kiểu trung thực paper-fill đã áp dụng cho sự cố cirBTC. Bước tiếp là productionize để **settle on-chain** (qua Circle Gateway) và ký bằng Circle Developer-Controlled Wallet của agent; struct đã ký sẵn sàng để settle.
